...
#  Look for -d, -f, -h and -? options
#  Exit after showing help if -? is given
...
# Make sure the required directories exist for Bird
mkdir -p $ETCPATH
mkdir -p $LOGPATH
mkdir -p $RUNPATH
mkdir -p $LOCKPATH
...
cfile="${ETCPATH}/bird-${handle}.conf" # Main config file
dest="${cfile}.$$"
